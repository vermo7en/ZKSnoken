...
contract Pausable is Ownable {
    //  creates a private '_paused' variable of type bool
    //  creates a public setter using the inherited onlyOwner modifier
    //  creates an internal constructor that sets the _paused variable to false
    //  creates 'isUnpaused' & 'paused' modifier that throws in the appropriate situation
    //  creates a Paused & Unpaused event that emits the address that triggered the event
    ...
    function pause() public onlyOwner isUnpaused {
        _paused = true;
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        // adds 'to' address to token approvals
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        // mints tokenId to given address & increase token count of owner
...
contract ERC721Enumerable is ERC165, ERC721 {
    ...
    // Maps from token ID to index of the owner tokens list
     ...
     */
    constructor () public {
        // registers the supported interface to conform to ERC721Enumerable via ERC165
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        // To prevent a gap in from's tokens array, stores the last token in the index of the token to delete, and
        // then deletes the last slot

        uint256 lastTokenIndex = _ownedTokens[from].length.sub(1);
        uint256 tokenIndex = _ownedTokensIndex[tokenId];

        // Checks if swap not needed
        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = _ownedTokens[from][lastTokenIndex];

            // Moves the last token to the to-delete token
    ...
    function _removeTokenFromAllTokensEnumeration(uint256 tokenId) private {
        ...
        // rarely (when the last minted token is burnt) that we still swap to avoid the gas cost of adding
        // an 'if' statement
...
// Moves the last token to the slot of the to-delete token
        ...
        // Deletes the contents at the last array position
        _allTokens.length--;
...
//  makes the base token uri: https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/
//  creates a public mint() that does the following:
//  only executed by the contract owner
